...
class FateCPSATSolver:
    ...
    def _analyze_flying_stars(self):
        ...
        for p in palaces_data:
            ...
            self_trans = []
            if p_name and target_lu != "外宮" and target_lu == p_name: 
                self_trans.append(f"自化祿({trans['lu']})")
            if p_name and target_quan != "外宮" and target_quan == p_name: 
                self_trans.append(f"自化權({trans['quan']})")
            if p_name and target_ke != "外宮" and target_ke == p_name: 
                self_trans.append(f"自化科({trans['ke']})")
            if p_name and target_ji != "外宮" and target_ji == p_name: 
                self_trans.append(f"自化忌({trans['ji']})")
    ...
    def _handle_teaching(self, name, clean_q=""):
        """專屬宗師傳道授業 · 紫微斗數排盤與四化飛星大白話講堂"""
        return (
            f"【紫微天機道長 · 30秒極速秒懂紫微斗數（白話生活版）】：\n\n"
            f"緣主 {name} 您好！老道今天不用一句艱澀古文，用「拍電影、開公司、寄快遞」帶你秒懂紫微斗數：\n\n"
            f"------------------------------------------------------------\n"
            f"🎬 【三大要素生活大比喻】\n"
            f"1. 🏠 **十二宮位 = 12 間生活房間**：辦公室（事業）、錢包（財帛）、臥室（夫妻）、客廳（交友）。告訴你事情在哪裡發生！\n"
            f"2. 🎭 **十四主星 = 14 位性格演員**：紫微（霸道總裁）、天機（諸葛軍師）、武曲（精算會計）、天同（吃貨甜心）。告訴你是誰在做事、脾氣如何！\n"
            f"3. 📦 **四化飛星 = 老天爺寄送的快遞包裹**：化祿（發財大紅包）、化權（握方向盤掌權）、化科（得貴人金牌）、化忌（心裡最放不下的責任牽掛）！\n\n"
            f"------------------------------------------------------------\n"
            f"💡 【飛星因果看盤法】：\n"
            f"宮位門口掛著天干，天干一動就像寄出包裹：\n"
            f"● 命宮寄化祿到財帛宮 ➔ 翻譯：「你自己超愛求財，財神爺也喜歡把錢包塞滿給你！」\n"
            f"● 夫妻宮寄化忌到命宮 ➔ 翻譯：「另一半把你放在第一位，但管得太緊、愛囉嗦讓你壓力山大！」\n"
            f"● 自化（自己寄給自己） ➔ 翻譯：「佛系隨緣、不記仇、情緒自己消化消化就過去了。」\n\n"
            f"想要親自動手體驗推盤樂趣，可點擊上方導航進入【紫微大講堂與飛星沙盒實驗室】！"
        )

    def _handle_flying_stars(self, name, clean_q=""):
        """專屬宗師四化飛星因果神斷 · 100% 生活白話版"""
        ...
        for f in flying_matrix:
            if f['self_trans'] and f['source']:
                joined_st = '、'.join(f['self_trans'])
                all_self_trans.append(f"{f['source']}有【{joined_st}】")

        self_trans_desc = '；'.join(all_self_trans) if all_self_trans else '你的星盤能量非常凝聚，沒有向外消散的自化現象，做事專注力極佳！'
        ...
        if ming_fly:
            ...
            m_quan_target = '官祿宮'
            m_ke_target = '福德宮'
            ming_trans = {'lu': '破軍', 'quan': '巨門', 'ke': '太陰', 'ji': '貪狼'}

        # 祿之因果（大白話）
        if m_lu_target == '財帛宮':
            lu_plain = f"你天生是個「賺錢嗅覺敏銳」的實幹家！自己對求財極度有熱忱與點子，老天爺也常把財運大紅包直接送到你錢包裡，求財路子特別順遂。"
        elif m_lu_target in ['官祿宮', '事業宮']:
            lu_plain = f"你是個「工作狂熱派」！熱愛在事業舞台上展現自我，只要一投入工作就能看到回報，在職場上特別受長官與合作夥伴喜愛。"
        elif m_lu_target == '夫妻宮':
            lu_plain = f"你對伴侶「超級好、捨得為對方花錢與付出」！在感情中是個體貼的付出型人格，樂於照顧另一半的生活細節。"
        elif m_lu_target == '田宅宮':
            lu_plain = f"你非常有「置產存房」的眼光！賺到的錢最喜歡沉澱為房子、不動產或大金庫，能把財富穩穩鎖在家庭裡造福全家。"
        elif m_lu_target == '遷移宮':
            lu_plain = f"你適合「往外走、出差旅行或跨界拓展」！你出門在外人緣極佳，到外地或新環境特別容易遇到提攜你的貴人。"
        elif m_lu_target == '命宮':
            lu_plain = f"你天性「樂觀隨和、懂得犒賞自己」！遇事不鑽牛角尖，心寬福厚，很懂得享受生活的美好。"
        else:
            lu_plain = f"你把你這輩子最豐厚的善意、資源與好心情，全部主動分享給【{m_lu_target}】這個生活領域，在此處最容易收穫滿滿的成就感！"

        # 權之因果（大白話）
        quan_plain = f"你在【{m_quan_target}】領域說話極有份量與威嚴！你做事果斷、有主見、敢於扛責任，大家都願意聽你指揮與帶領。"

        # 科之因果（大白話）
        ke_plain = f"你在【{m_ke_target}】領域自帶「貴人光環與優雅口碑」！遇到困難總能遇難呈祥、逢凶化吉，靠著才華與好名聲贏得大家尊重。"

        # 忌之因果（大白話）
        if m_ji_target == '命宮':
            ji_plain = f"你對自己「自我要求極高、容易在內心跟自己較勁」！心裡容易胡思亂想，學會『放過自己、多讚美自己』是你今生最大的開運心法。"
        elif m_ji_target == '遷移宮':
            ji_plain = f"你這輩子「最操心、最耗神」的地方在【出外打拼與人際應酬】！在外面奔波常覺得身心疲憊或感到孤軍奮戰，在外言行宜低調穩健，不要急躁或多管閒事即可化解。"
        elif m_ji_target == '財帛宮':
            ji_plain = f"你對「金錢與財務安全感」特別在乎與操心！容易為開銷預算焦慮，只要做好定期定額理財規劃，就能安心。"
        elif m_ji_target in ['官祿宮', '事業宮']:
            ji_plain = f"你工作責任感太重，容易「把工作當作生活全部而過勞」！需要提醒自己勞逸結合，多留時間陪伴家人。"
        elif m_ji_target == '夫妻宮':
            ji_plain = f"你在感情中「愛得太深、太在乎對方的一舉一動」！容易因小事吃醋或猜疑，給彼此多一點信任與私人空間就是幸福密碼。"
        elif m_ji_target == '田宅宮':
            ji_plain = f"你對家庭家人「責任心極重、什麼事都想一肩扛起」！凡事親力親為，多讓家人分擔家務，心裡會更輕鬆。"
        else:
            ji_plain = f"【{m_ji_target}】是你這輩子「最放不下心、投入最多精力」的牽掛所在！多看淡得失，順其自然反而能收穫大智慧。"

        return (
            f"【紫微天機道長 · 飛星生活大白話解密】：\n\n"
            f"緣主 {name} 請看！這張星盤就是你的人生劇本，老道用最接地氣的大白話為你拆解：\n\n"
            f"------------------------------------------------------------\n"
            f"📦 【一、你的命宮（{m_gan}干）寄出了四個命運包裹】：\n\n"
            f"🌸 1. **化祿包裹（送大紅包）➔ 寄給【{m_lu_target}】（引動{ming_trans['lu']}星）**\n"
            f"   👉 **大白話人話**：{lu_plain}\n\n"
            f"⚡ 2. **化權包裹（遞方向盤）➔ 寄給【{m_quan_target}】（引動{ming_trans['quan']}星）**\n"
            f"   👉 **大白話人話**：{quan_plain}\n\n"
            f"📜 3. **化科包裹（發貴人金牌）➔ 寄給【{m_ke_target}】（引動{ming_trans['ke']}星）**\n"
            f"   👉 **大白話人話**：{ke_plain}\n\n"
            f"🌀 4. **化忌包裹（心裡的牽掛）➔ 寄給【{m_ji_target}】（引動{ming_trans['ji']}星）**\n"
            f"   👉 **大白話人話**：{ji_plain}\n\n"
            f"------------------------------------------------------------\n"
            f"💡 【二、什麼是全盤自化？（自己消化情緒）】：\n"
            f"● **你的自化現象**：{self_trans_desc}\n"
            f"👉 **大白話人話**：自化就像「自己寄包裹給自己」。代表在該生活領域裡，你個性比較隨性豁達，不愛跟人計較，發生什麼事情通常自己在心裡消化一下就過去了！\n\n"
            f"------------------------------------------------------------\n"
            f"🎯 【三、道長給你 3 條最實用的生活破局建議】：\n"
            f"1. **【跟著財氣走】**：你的賺錢福祿就在【{m_lu_target}】，多把心思和時間放在這上面，財神爺自然常來敲門！\n"
            f"2. **【出外放寬心】**：面對【{m_ji_target}】的操心事，記住『盡人事、聽天命』，不要凡事自己硬扛，學會釋懷就能逢凶化吉。\n"
            f"3. **【每日開運吉時與吉方】**：\n"
            f"   - 最佳沈澱開運時辰：每日【{getattr(self, 'best_timing', None) or '子時 (23:00 - 01:00)'}】\n"
            f"   - 最佳迎祥吉利方位：面朝【{getattr(self, 'best_direction', None) or '正北方 (喜水納氣)'}】放鬆深呼吸。\n\n"
            f"💡 **一句話總結**：『愛賺錢也懂得賺錢，出外打拼別給自己太大壓力，多笑一笑，好運自然跟著來！』"
        )
