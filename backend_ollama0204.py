...
from flask import Flask, request, jsonify, make_response, send_file, Response, stream_with_context
...
class BackendApp(tk.Tk):
    ...
    def start_server(self):
        ...
        self.lbl_server.config(text="● 伺服器: 運行中 (Port 5000)", foreground="#4ade80")
...
GEMINI_API_KEY = "AIzaSyDHKuL1Sgj-04yNrlOookpfBaeveSZeO4E" # 使用您的 Gemini API Key
...
def call_gemini_api(prompt, system_prompt="", stream=True):
    ...
    # 使用診斷確認可用的 2026 年主流模型：Gemini 2.5 Flash (v1 Stable)
    ...
    for attempt in range(max_retries):
        try:
            if stream:
                # Gemini 的串流格式與 Ollama 不同，這裡我們先做非串流模擬或使用其串流 endpoint
                # 為了簡易起見，我們先實作標準 POST 並一次 yield 回來 (Gemini 1.5 Flash 很快)
            ...
            if e.response.status_code == 429:
                wait_time = (attempt + 1) * 3  # 3s, 6s, 9s... 更溫和的重試
                print(f"Gemini API 429 Too Many Requests (Attempt {attempt+1}/{max_retries}). Retrying in {wait_time}s...")
            ...
            print(f"Gemini API 連線錯誤: {e}")
            return None
            
    print("Gemini API Retry Limit Exceeded.")
    return None
