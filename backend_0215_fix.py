...
# --- FORCE IPV4 PATCH ---
# 解決部分網路環境因 IPv6 導致 SSL 連線異常 (Unexpected EOF) 的問題
import socket
import urllib3.util.connection as urllib3_cn

def allowed_gai_family():
    """
    Force IPv4 family for urllib3 connections.
    """
...
class BackendApp(tk.Tk):
    ...
    def check_ollama_status(self):
        try:
            requests.get("http://localhost:11434", timeout=1, verify=False)
...
def load_config():
    ...
    defaults = {
        ...
        "gemini": {"api_key": "AIzaSyBEVWhdoibHyuw56osfTbgGFSZ2D9iP3bk", "model": "gemini-1.5-flash"},
...
def detect_intent_and_context(prompt, chart_data):
    ...
    # EXCEPT for Bazi queries where we want to keep it focused on 4-pillars
...
請開始為緣主批命。"""
    ...
    def generate_unified_response():
        yield "SYSTEM: Running Backend Ollama 2015 - VERSION DEBUG 999\n"
        ...
        if is_daily_query:
            ...
            if (yield from call_ai_engine(summary_prompt, short_system_prompt)):
                return
...
語氣需神秘、深邃且充滿慈悲。字數約 300 字。強制繁體中文。"""
            ...
            yield "【三世因果解碼報告】\n"
            yield "------------------------------------------\n"
            if (yield from call_ai_engine(summary_prompt, karma_system_prompt)):
                return
...
語氣需正向、神聖且具體可行。字數約 300 字。強制繁體中文。"""
            ...
            yield "【道家轉運開運儀式】\n"
            yield "------------------------------------------\n"
            if (yield from call_ai_engine(summary_prompt, ritual_system_prompt)):
                return
...
語氣需溫柔、療癒且具洞察力。字數約 300 字。強制繁體中文。"""
            ...
            yield "【紫微潛意識夢境解碼】\n"
            yield "------------------------------------------\n"
            if (yield from call_ai_engine(summary_prompt, dream_system_prompt)):
                return
...
【絕對禁止】：禁止提及任何「紫微斗數」的星曜（如紫微、天機、擎羊、化忌等）或宮位名稱（如夫妻宮、身宮）。
你的任務是「純粹」分析緣主的四柱八字。
1. **八字格局**：分析日主五行強弱，定出格局（如印綬格、財官格）。
2. **喜用神分析**：明確指出喜忌五行。
3. **十神論斷**：依據天干地支生剋合化，推算事業財運與婚姻。
語氣需古樸專業，字數約 400 字。強制繁體中文。"""
            
            summary_prompt = f"請根據以下命盤中的【八字資訊】，進行子平八字論命：\n{clean_prompt}"
            
            yield "【子平八字精批】\n"
            yield "------------------------------------------\n"
            if (yield from call_ai_engine(summary_prompt, bazi_system_prompt)):
                return
...
語氣需活潑、犀利又帶點幽默。字數約 300 字。強制繁體中文。"""
            ...
            yield "【紫微戀愛桃花攻略】\n"
            yield "------------------------------------------\n"
            if (yield from call_ai_engine(summary_prompt, love_system_prompt)):
                return
...
語氣需專業、務實且具前瞻性。字數約 300 字。強制繁體中文。"""
            ...
            yield "【紫微財運投資佈局】\n"
            yield "------------------------------------------\n"
            if (yield from call_ai_engine(summary_prompt, finance_system_prompt)):
                return
        ...
        if matched_results and is_full_report:
            yield "【天機運算中...】大師正在逐條詳批您的命盤格局，請稍候...\n\n"
            yield "【紫微命譜格局逐條精批】\n"
            yield "------------------------------------------\n"
            
            # System prompt: Focused on explaining ONE specific rule clearly
...
請直接回答，不要重複題目，字數 50-80 字。"""
            ...
            # Process ALL rules sequentially (Limit to 20 for time)
            for i, res in enumerate(matched_results[:20]):
                ...
                # 2. Ask AI to explain THIS SPECIFIC rule immediately
                if GEMINI_API_KEY:
                    # Specific prompt for this rule
                    mini_prompt = f"請解釋紫微斗數格局：「{desc}」。\n格局內容：「{text}」。\n這代表什麼意思？"
                    
                    try:
                        time.sleep(1) # Rate limit protection
                        # Call API just for this rule - with specific error logging
                        explanation = call_gemini_api(mini_prompt, explanation_system_prompt)
                        
                        if explanation:
                            yield f"  ↳ 大師解讀：{explanation}\n"
                        else:
                            yield f"  (大師沈默...可能是API未回傳內容)\n"
                    except Exception as e:
                        yield f"  (連線異常: {str(e)})\n"
                else:
            ...
            return # End here, do NOT generate summary

        # --- AI 綜合結論部分 (Summary Only) - Only runs if NOT full report mode ---
        # 如果不是詳評模式，才執行下面的簡短對話邏輯
        summary_prompt = f"""請根據以下數據回覆緣主的問題：
{user_prompt}"""
        
        # --- 正常對話模式 ---
        if (yield from call_ai_engine(summary_prompt, full_system_prompt)):
            return
